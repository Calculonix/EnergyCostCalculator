...
export function initApplianceCalculatorPage({ defaultPresetId = '' } = {}) {
  ...
  function createInstance(form, index, presetId = '') {
    ...
    if (presetId) applyPreset(); else update();
    return instance;
  }
  ...
  createInstance(firstForm, 0, defaultPresetId);
  populatePresets();
  ...
  document.querySelector('#reset-button').addEventListener('click', () => { while (instances.length > 1) removeInstance(instances.length - 1); const instance = instances[0]; instance.form.reset(); instance.sliders.hours.value = calculatorDefaults.hoursPerDay; instance.sliders.days.value = calculatorDefaults.daysPerWeek; instance.presetSelect.value = defaultPresetId; if (defaultPresetId) { instance.fields.hours.value = calculatorDefaults.hoursPerDay; instance.fields.days.value = calculatorDefaults.daysPerWeek; instance.fields.price.value = defaultElectricityPrice; instance.applyPreset(); } else { instance.fields.watts.value = ''; instance.fields.hours.value = ''; instance.fields.days.value = ''; instance.fields.price.value = ''; instance.fields.watts.dispatchEvent(new Event('input', { bubbles: true })); } });
  const heroWatts = document.querySelector('#hero-watts');
  const heroUsage = document.querySelector('#hero-usage');
